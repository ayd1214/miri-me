...
async def analyze_image_with_ai(image: UploadFile) -> dict:
    ...
    try:
        ...
        prompt_text = f"""이 이미지는 학교 과제 공지이며 당신은 이를 분석하여 구조화된 데이터를 생성하는 AI입니다. 오늘 날짜와 시간은 {current_time} 입니다. 요일만 적혀있다면 제공된 {upcoming_dates}를 참고하세요.
...
4. 키워드 (keywords): 
   ...
   - 추출된 각 키워드는 유저가 즉각적으로 행동 지침을 파악할 수 있도록 구절 단위로 작성하세요. 각 항목 앞에 ' - ' 마크를 붙여 리스트 요소로 만드세요.
