...
3. 우선순위(priority): 마감일(긴급도)보다 과제의 내용(학점 반영 여부, 난이도, 노력)을 최우선으로 판단하여 "high", "medium", "low" 중 하나로 산정하세요. 아무리 마감일이 지났거나 임박했어도, 단순 안내문이나 간단한 신청이라면 무조건 low를 부여해야 합니다.
   - high: 배점(Points)이 높거나, 제출 방식이 까다롭고 시간이 오래 걸리는 경우(예: 동영상 제작, 유튜브 업로드, 발표, 팀 프로젝트), 또는 치명적인 조건(선착순 등)이 있는 경우.
   - medium: 일반적인 노력과 시간이 들어가는 표준적인 과제(예: 일반 코딩, 짧은 레포트).
   - low: 배점이 없거나 아주 낮은 경우, 단순 텍스트 입력, 출석 체크, 혹은 독서실 신청 같은 **단순 안내문/신청문**인 경우 (마감일 임박 여부와 무관하게 무조건 low 부여).
