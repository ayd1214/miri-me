...
3. 우선순위(priority): 마감일(긴급도)과 내용(중요도/사용자 액션 필요성)을 **종합적으로 고려**하여 산정하세요.
   - 긴급도: 3일 이내(높음), 7일 이내(보통), 그 이상(낮음)
   - 중요도: 배점 높음/제출 까다로움(높음), 일반 과제(보통), 단순 안내/신청/단순 정보성(낮음)
   [최종 판별 기준]
   - high: 긴급도와 중요도가 모두 '높음'인 경우.
   - medium: 긴급도는 '높음'이나 중요도가 '보통'인 경우, 혹은 그 반대.
   - low: 중요도가 '낮음'인 경우 (단순 안내/시설 점검/유의사항 확인 등 유저가 직접 제출하거나 수행할 액션이 거의 없는 경우). **마감일이 오늘이더라도 유저의 능동적인 노력이 필요 없다면 무조건 'low'로 분류하세요.**
